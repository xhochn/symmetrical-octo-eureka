...
{
    [Info("Server Armour", "Pho3niX90", "0.6.40")]
    ...
    class ServerArmour : CovalencePlugin
    {
        ...
        void SCmdBan(IPlayer player, string command, string[] args) {
            Puts($"BAN RAN: {command}");
            Puts($"args: {String.Join(",", args)}");
        ...
        bool HasReachedTotalBanCeiling(ISAPlayer isaPlayer)
        {
            ...
            bool kick = config.AutoKickOn && config.EnableTotalBanKick && config.AutoTotalBanCeiling >= 0;
            if (kick)
            {
